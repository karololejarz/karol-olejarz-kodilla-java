...
public class CrudAppTestSuite {
    private static final String BASE_URL = "https://karololejarz.github.io/github.io/";
    ...
    /*when tested as void works*/
    public String createCrudAppTask() throws InterruptedException {
    ...
    private boolean checkTaskExistsInTrello(String taskName) throws InterruptedException {
        ...
        driverTrello.findElement(By.id("user")).sendKeys("karol1olejarz@gmail.com");
    ...
    public boolean cleanUpTask(String taskName) throws InterruptedException {
        //We wait for elements
        driver.navigate().refresh();
        while (!driver.findElement(By.xpath("//select[1]")).isDisplayed()) ;
        ...
        driver.findElements(By.xpath("//form[@class=\"datatable__row\"]")).stream()
                .filter(anyForm -> anyForm.findElement(By.xpath(".//p[@class=\"datatable__field-value\"]")).getText().equals(taskName))
                .forEach(theForm -> {
                    //this is the only 4th button in page, has no separate class
                    WebElement buttonDeleteTask = theForm.findElement(By.xpath(".//button[4]"));
                    buttonDeleteTask.click();
                });

        Thread.sleep(5000);
        //We can return true, if anything goes wrong the console would detect it
        return true;
    }
    ...
    public void shouldCreateTrelloTask() throws InterruptedException {
        ...
        //Trello alerts about card created causing org.openqa.selenium.UnhandledAlertException
        //We accept the alert, without handling this exception
        driver.switchTo().alert().accept();
        Assert.assertTrue(checkTaskExistsInTrello(taskName));
        //Asserts that cleanup went without problems
        Assert.assertTrue(cleanUpTask(taskName));
    }
}
